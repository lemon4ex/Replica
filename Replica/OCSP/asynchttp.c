...
//        secdebug("http", "GET[%ld] %@", CFURLGetBytes(getURL, NULL, 0), getURL);
        http->request = CFHTTPMessageCreateRequest(kCFAllocatorDefault,
                                                   CFSTR("GET"), getURL, kCFHTTPVersion1_1);
    } else {
...
//        secdebug("http", "POST %@ CRLF body", responder);
        http->request = CFHTTPMessageCreateRequest(kCFAllocatorDefault,
                                                   CFSTR("POST"), responder, kCFHTTPVersion1_1);
...
//    secdebug("http", "request %@", request);
    ...
    http->stream = CFReadStreamCreateForHTTPRequest(
                                                    kCFAllocatorDefault, http->request);

	/* Set a reasonable timeout */
    http->timer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, http->queue);
